        ...
        <div className="lists">
          {Years.length !== 0 ? (
            Years.map((yearEach, yearIdx) => {
              ...
              return (
                <div
                  key={`${yearEach.id}${yearIdx}`}
                  className="lists__year-wrap"
                >
                  <div className="lists__year">{year}년</div>
                  <div className="lists__month-wrap">
                    {monthArray.map((monthEach, monthIdx) => {
                      ...
                      return (
                        <div
                          key={`month${monthIdx}`}
                          className="lists__month-item"
                        >
                          <div className="lists__month">{month}월</div>
                          <div className="lists__list-wrap">
                            {listsArray.map((listsEach, listsIdx) => {
                              ...
                              return (
                                <NavLink
                                  key={`lists${listsIdx}`}
                                  to={`/view/${listsEach.id}?year=${year}&month=${month}`}
                                  className="lists__list-item"
                                >
                                  <div className="lists__date-wrap">
                                    ...
                                    <div className="lists__icon-wrap">
                                      {listsEach.lists.medicine === "yes" && (
                                        <img
                                          src={`${publicFolder}/assets/img/icon/medicine.png`}
                                          alt="약 아이콘"
                                          className="lists__icon"
                                        />
                                      )}
                                      {listsEach.lists.shot === "yes" && (
                                        <img
            ...
            <div className="lists__none-wrap">
              ...
              <div className="lists__none-title">일지를 작성해 보세요</div>
            </div>
          )}
        </div>
      ) : (
