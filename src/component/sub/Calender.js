  ...
  return (
    <Layout>
      <div className="calender__nav-wrap">
        <div className="calender__nav s-active">
            달력
        </div>
      ...
      {!Graph && <Alert type="default" clickHandler={btnOk} title="확인해 주세요" content="준비중 입니다." />}
      {DimOpen && <Dim setDimOpen={setDimOpen} setDefault={setGraph} setSubmit={setGraph} />}
      <Calendar
        onChange={setChange}
        value={Change}
      ...
      <div className="calender-detail">
        {Medicine &&
          <div className="calender-detail__item">
            <div className="calender-detail__title">
              <img src={`${publicFolder}/assets/img/icon/medicine.png`} alt="약 아이콘" className="calender-detail__icon" />
              약 복용 날짜
            </div>
            <div className="calender-detail__date-wrap">
              {Medicine.map((medi, mediIdx) => {
                if(MedicineLink.length !== 0) {
                  return (
                    <NavLink key={DataMedicineKey[mediIdx]} className="calender-detail__date" to={`/view/${DataMedicineKey[mediIdx]}?year=${MedicineLink[mediIdx].year}&month=${MedicineLink[mediIdx].month}`}>
                      {medi}
                    </NavLink>
                  )
                }
        ...
        {Shot &&
          <div className="calender-detail__item">
            <div className="calender-detail__title">
              <img src={`${publicFolder}/assets/img/icon/shot.png`} alt="주사 아이콘" className="calender-detail__icon" />
              주사 맞은 날짜
            </div>
            <div className="calender-detail__date-wrap">
              {Shot.map((sht, shtIdx) => {
                if(ShotLink.length !== 0) {
                  return (
                    <NavLink key={DataShotKey[shtIdx]} className="calender-detail__date" to={`/view/${DataShotKey[shtIdx]}?year=${ShotLink[shtIdx].year}&month=${ShotLink[shtIdx].month}`}>
                      {sht}
                    </NavLink>
                  )
                }
